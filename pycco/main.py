...
def generate_documentation(
    source, outdir=None, preserve_paths=True, md=False, language=None, encoding="utf8"
):
    """
    ...
    return _generate_documentation(source, code, outdir, preserve_paths, md, language)


def _generate_documentation(file_path, code, outdir, preserve_paths, md, language):
    """
...
def process(
    ...
    if sources:
        ...
        def next_file():
            s = sources.pop(0)
            dest = destination(s, preserve_paths=preserve_paths, outdir=outdir, md=md)
...
def main():
    ...
    parser.add_argument(
        "-m", "--markdown", action="store_true", help="Output Markdown instead of HTML"
    )
